...
TRAINED_MASKRCNN_BUILDINGS_WEIGHTS = "mark_rcnn_buildings_0030.h5"
CLASS_TREE = "Tree"
CLASS_BUILDINGS = "Building"
IMG_DIR = "testimage"
INFERED_DIR = "testimageinfer"

class InferenceConfig(maskrcnntrain.trainingconfig):
    GPU_COUNT = 1
    IMAGES_PER_GPU = 1
...
def main():
    ...
    for filename in os.listdir(IMG_DIR):
        ...
        if selection == 0:
            ...
            r = results[0]
            infered, number_of_instances = display_instances(
                test_image, r['rois'], r['masks'], r['class_ids'], class_names_building, r['scores'], [102, 255, 255]
            )
            print(number_of_instances)
            ...
            test_image = cv2.imread(os.path.join(infered_temp,filename))
            results = model_building.detect([test_image], verbose = 0)
            r = results[0]
            infered_temp, number_of_building_instances = display_instances(
                test_image, r['rois'], r['masks'], r['class_ids'], class_names_building, r['scores'], [102, 255, 255]
            )
            print(number_of_tree_instances)
            print(number_of_building_instances)
